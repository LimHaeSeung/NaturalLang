...
with tf.Session(graph=graph) as session:
    ...
    for qword in query_words:
        query_word_ids.append(word2id[qword])
    query_dataset = tf.constant(query_word_ids, dtype=tf.int32)  # word_ids of validation words

    query_embeddings = tf.nn.embedding_lookup(normalized_embeddings, query_dataset)
    query_similarity = tf.matmul(query_embeddings, normalized_embeddings, transpose_b=True)

    query_sim = query_similarity.eval()  # dimension: (20, 50000)
    for i in range(len(query_words)):
        top_k = 8  # number of nearest neighbors
        nearest = (-query_sim[i, :]).argsort()[1:top_k + 1]

        log_str = 'Nearest to %s:' % query_words[i]
        sim_log_str = ''
        outfile.write(query_words[i] + '\n')
        for k in range(top_k):
            close_word = id2word[nearest[k]]
            log_str = '%s %s,' % (log_str, close_word)
            sim_log_str = '%s %.4f' % (sim_log_str, query_sim[i, nearest[k]])
            outfile.write('%.4f %s\n' % (query_sim[i, nearest[k]], close_word))
        print(log_str)
        print(sim_log_str)
        outfile.write('\n')
